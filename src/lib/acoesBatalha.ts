...
export async function fetchQuantidadeAcoes(userId: string, data: string): Promise<Map<string, LogQuantidade>> {
  const { data: rows, error } = await supabase
    .from("habito_logs" as any)
    .select("habito_id, quantidade_atual, completado")
    .eq("user_id", userId)
    .eq("data", data);
  if (error) throw error;
  return new Map((rows ?? []).map((r: any) => [r.habito_id, { quantidadeAtual: Math.max(0, Number(r.quantidade_atual ?? 1)), completado: !!r.completado }]));
}

export async function executarAcaoBatalha(opts: { heroi: Heroi; inimigo: Inimigo | null; habito: Habito; data?: string; quantidadeAtual: number; marcado: boolean; }) {
  ...
  if (porQuantidade) {
    ...
    if (atual === 0) {
      const { error } = await supabase.from("habito_logs" as any).insert({ user_id: heroi.id, habito_id: habito.id, data: dia, completado: novaQuantidade >= meta, quantidade_atual: novaQuantidade });
      if (error) throw error;
    } else {
      const { error } = await supabase.from("habito_logs" as any).update({ quantidade_atual: novaQuantidade, completado: novaQuantidade >= meta }).eq("habito_id", habito.id).eq("data", dia);
      if (error) throw error;
    }
  ...
  if (marcado) {
    const { error } = await supabase.from("habito_logs" as any).delete().eq("habito_id", habito.id).eq("data", dia);
    if (error) throw error;
  } else {
    const { error } = await supabase.from("habito_logs" as any).insert({ user_id: heroi.id, habito_id: habito.id, data: dia, completado: true, quantidade_atual: 1 });
    if (error) throw error;
  }
...
async function aplicarEfeitoBatalha(opts: { heroi: Heroi; inimigo: Inimigo | null; habito: Habito; positivo: boolean; skill: ReturnType<typeof getPetSkill>; sign: number; }) {
  ...
  if (ouroDelta !== 0) { const { error } = await supabase.from("transacoes_ouro").insert({ user_id: heroi.id, valor: ouroDelta, origem: "habito", descricao: habito.nome }); if (error) throw error; }
  ...
  emitGameEvent(sign > 0 ? "HABIT_COMPLETED" : "HABIT_UNDONE", { habitId: habito.id, habitName: habito.nome, xpDelta, goldDelta: ouroDelta }, "executarAcaoBatalha");
  if (enemyDefeated && inimigo) emitGameEvent("ENEMY_DEFEATED", { enemyId: inimigo.id, enemyName: inimigo.nome }, "executarAcaoBatalha");
