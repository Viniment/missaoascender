...
export default function Shell({ children }: { children: React.ReactNode }) {
  ...
  const renderNav = (mobile = false) => (
    <div className={cn("space-y-3", mobile && "space-y-4", !mobile && !desktopExpanded && "space-y-7")}>
      {Object.entries(grouped).map(([group, items]) => (
        <section key={group}>
          <div className={cn("mb-2 flex items-center gap-2 px-2", !mobile && !desktopExpanded && "mb-4 justify-center px-0")}>
          ...
          <div className={cn("space-y-1", !mobile && !desktopExpanded && "space-y-4")}>
            {items.map(({ to, label, Icon }) => {
              ...
              return (
                <Link key={to} to={to} onClick={mobile ? closeMenu : undefined} aria-label={label} title={!mobile && !desktopExpanded ? label : undefined}
                  className={cn("group/item relative flex items-center overflow-hidden border transition-all duration-200", mobile ? "min-h-11 gap-3 rounded-xl px-3" : desktopExpanded ? "h-10.5 gap-3 rounded-xl px-2.5" : "mx-auto h-16 w-16 justify-center rounded-xl p-0",
                  ...
                  {active && !(!mobile && !desktopExpanded) && <span className="absolute left-0 top-1/2 h-8 w-0.5 -translate-y-1/2 bg-primary shadow-[0_0_10px_hsl(var(--primary)/0.95)]" />}
                  {active && !mobile && !desktopExpanded && <span className="pointer-events-none absolute inset-0 rounded-xl border border-primary/55 bg-primary/10 shadow-[0_0_22px_hsl(var(--primary)/0.2),inset_0_0_18px_hsl(var(--primary)/0.08)]" />}
                  <span className={cn("relative z-10 flex shrink-0 items-center justify-center border transition-all", mobile ? "h-8 w-8 rounded-lg" : desktopExpanded ? "h-8 w-8 rounded-lg" : "h-12 w-12 rounded-lg p-2.5", active ? "border-primary/50 bg-primary/20 text-primary shadow-[0_0_12px_hsl(var(--primary)/0.15)]" : "border-border/40 bg-background/30 group-hover/item:border-primary/30 group-hover/item:bg-primary/10 group-hover/item:text-primary")}>
                    <Icon className={cn(mobile ? "h-4 w-4" : desktopExpanded ? "h-[18px] w-[18px]" : "h-[23px] w-[23px]")} />
                  </span>
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <aside className={cn("fixed left-0 top-14 bottom-0 z-40 hidden border-r border-primary/15 bg-background/82 backdrop-blur-2xl transition-[width,box-shadow] duration-300 ease-out lg:flex lg:flex-col xl:top-16", desktopExpanded ? "w-[250px] shadow-[14px_0_45px_rgba(0,0,0,.24)]" : "w-[104px]")}>
        <div className="flex h-full w-full min-h-0 flex-col px-3 py-3">
          <div className="min-h-0 flex-1 overflow-hidden px-1">
            <div className={cn("new-lifeup-scroll h-full overflow-y-auto py-3", desktopExpanded ? "pr-4 pl-1" : "px-2.5 pr-5")}>{renderNav()}</div>
          </div>
      ...
      <main className={cn("relative z-10 min-h-[calc(100vh-3.5rem)] px-3 pb-10 pt-4 transition-[padding] duration-300 sm:px-4 sm:pt-6 xl:min-h-[calc(100vh-4rem)]", desktopExpanded ? "lg:pl-[270px]" : "lg:pl-[124px]")}>{children}</main>
    </div>
  );
}
