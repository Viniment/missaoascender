  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <aside className={cn("fixed left-0 top-14 bottom-0 z-40 hidden border-r border-primary/15 bg-background/82 backdrop-blur-2xl transition-[width,box-shadow] duration-300 ease-out lg:flex lg:flex-col xl:top-16", desktopExpanded ? "w-[250px] shadow-[14px_0_45px_rgba(0,0,0,.24)]" : "w-[88px]")}>
        <div className={cn("flex h-full w-full min-h-0 flex-col py-3", desktopExpanded ? "px-3" : "px-0")}>
          <div className={cn("min-h-0 flex-1 overflow-hidden", desktopExpanded ? "px-1" : "px-0")}>
            <div className={cn("new-lifeup-scroll h-full overflow-y-auto py-1", desktopExpanded ? "pr-3 pl-1" : "px-0")}>{renderNav()}</div>
          </div>
          <div className={cn("shrink-0 border-t border-primary/10 pt-2.5", desktopExpanded ? "mt-3" : "mt-3 mx-2")}>
