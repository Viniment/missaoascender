...
export default function Shell({ children }: { children: React.ReactNode }) {
            ...
            {items.map(({ to, label, Icon }) => {
              ...
              return (
                <Link key={to} to={to} onClick={mobile ? closeMenu : undefined} aria-label={label} title={!mobile && !desktopExpanded ? label : undefined}
                  ...
                  {active && !mobile && !desktopExpanded && <span className="pointer-events-none absolute inset-0 rounded-lg border border-primary/55 bg-primary/10 shadow-[0_0_22px_hsl(var(--primary)/0.2),inset_0_0_18px_hsl(var(--primary)/0.08)]" />}
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <main className={cn("relative z-10 min-h-[calc(100vh-3.5rem)] px-3 pb-10 pt-4 transition-[padding] duration-300 sm:px-4 sm:pt-6 xl:min-h-[calc(100vh-4rem)]", desktopExpanded ? "lg:pl-[270px]" : "lg:pl-[108px]")}>{children}</main>
    </div>
  );
}
