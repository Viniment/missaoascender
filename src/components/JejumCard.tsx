...
export default function JejumCard() {
  ...
  useEffect(() => {
    if (!uid) return;
    const load = async () => {
      const [savedSessions, savedActive, reset] = await Promise.all([
        getUserState<Session[]>(uid, SESSION_KEY, []),
        getUserState<ActiveFast | null>(uid, ACTIVE_KEY, null),
        supabase.from("users").select("jejum_reset_at").eq("id", uid).maybeSingle(),
      ]);
      const resetAt = reset.data?.jejum_reset_at ?? null;
      if (resetAt) {
        await Promise.all([deleteUserState(uid, SESSION_KEY), deleteUserState(uid, ACTIVE_KEY)]);
        setSessions([]); setActive(null);
      } else {
        setSessions(savedSessions); setActive(savedActive?.startedAt ? savedActive : null);
      }
    };
    void load();
  }, [uid]);
  ...
  const start = async () => {
    if (!uid) return;
    const iso = new Date(startValue).toISOString(); const next: ActiveFast = { startedAt: iso };
    try {
      await setUserState(uid, ACTIVE_KEY, next); setActive(next); setNow(Date.now()); setStartOpen(false); setExpanded(true); window.dispatchEvent(new Event("ascensao:jejum-active-changed"));
    } catch (error) { console.error(error); }
  };
  const openFinish = () => { setFinishValue(toInputValue(new Date())); setFinishOpen(true); };
  const confirmFinish = async () => {
    if (!active || !uid) return;
    const endedAt = new Date(finishValue).toISOString(); const minutes = getMinutes(active.startedAt, endedAt); if (minutes <= 0) return;
    const fullHours = Math.floor(minutes / 60); const xp = Array.from({ length: fullHours }, () => randomPerHour()).reduce((a, b) => a + b, 0); const ouro = Array.from({ length: fullHours }, () => randomPerHour()).reduce((a, b) => a + b, 0); const vida = fullHours; const atributoDelta = Array.from({ length: fullHours }, () => randomPerHour()).reduce((a, b) => a + b, 0); setSaving(true);
    try {
      const current = await fetchHeroi(uid); if (!current) throw new Error("Herói não encontrado.");
      const nextXp = applyXp(current, xp); const nextVida = Math.min(current.vida_max, current.vida_atual + vida);
      ...
      if (ouro > 0) { const { error } = await supabase.from("transacoes_ouro").insert({ user_id: uid, valor: ouro, origem: "jejum", descricao: `Jejum de ${durationText(minutes)}` }); if (error) throw error; }
      ...
      if (nextXp.conquistas.length) { const { error } = await supabase.from("conquistas").upsert(nextXp.conquistas.map(c => ({ user_id: uid, tipo: c.tipo, titulo: c.titulo, descricao: c.descricao })), { onConflict: "user_id,tipo", ignoreDuplicates: true }); if (error) throw error; }
      const reward: Reward = { xp, ouro, vida, atributo: "autodominio", atributoDelta, horas: fullHours };
      const session: Session = { id: crypto.randomUUID(), startedAt: active.startedAt, endedAt, minutes, reward };
      const nextSessions = [session, ...sessions];
      await setUserState(uid, SESSION_KEY, nextSessions);
      await deleteUserState(uid, ACTIVE_KEY);
      setSessions(nextSessions); setActive(null); setFinishOpen(false); window.dispatchEvent(new Event("ascensao:jejum-active-changed"));
  ...
  return <div className="rpg-panel overflow-hidden border-primary/30 shadow-[0_0_28px_hsl(var(--primary)/.06)]">
    ...
    <AnimatePresence>{finishOpen && <motion.div className="fixed inset-0 z-[90] bg-background/80 backdrop-blur-sm flex items-center justify-center p-4"><motion.div initial={{ scale: .96, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-md rpg-panel p-5 space-y-4"><div className="flex items-center justify-between"><h3 className="font-display tracking-widest">ENCERRAR JEJUM</h3><button onClick={() => setFinishOpen(false)}><X className="w-4 h-4" /></button></div><p className="text-xs text-muted-foreground">Confirme a data e a hora exatas, incluindo os segundos.</p><input type="datetime-local" step="1" value={finishValue} onChange={e => setFinishValue(e.target.value)} className="w-full bg-secondary border border-border rounded-md px-3 py-2 text-sm" /><button disabled={saving} onClick={() => void confirmFinish()} className="w-full btn-pixel py-2.5 rounded-md text-xs disabled:opacity-50">{saving ? "Salvando..." : "Confirmar Encerramento"}</button></motion.div></motion.div>}</AnimatePresence>
  </div>;
}
