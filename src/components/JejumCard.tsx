...
// Cada hora completa gera 1 nível de recompensa. O nível é sorteado entre 1 e 3;
// quanto maior o nível, mais raro e maior o pacote de recompensas.
...
export default function JejumCard() {
  ...
  useEffect(() => {
    ...
    if (raw) {
      try { setActive(JSON.parse(raw)); }
      catch { localStorage.removeItem(`${STORAGE(uid)}:active`); }
    }
  }, [uid]);
  ...
  const chooseMood = (value: Mood) => {
    ...
    const index = updated.hourlyStates.findIndex(x => x.hour === hour);
    const item = { hour, mood: value, at: new Date().toISOString() };
    if (index >= 0) updated.hourlyStates[index] = item;
    else updated.hourlyStates.push(item);
    setActive(updated);
  ...
  const confirmFinish = async () => {
    ...
    const levels = Array.from({ length: fullHours }, () => rollHourlyRewardLevel());
    const xp = levels.reduce((sum, level) => sum + level * 25, 0);
    const ouro = levels.reduce((sum, level) => sum + level * 8, 0);
    const vida = levels.reduce((sum, level) => sum + rollLifeReward(), 0);
    const atributoDelta = levels.reduce((sum, level) => sum + level, 0);
    ...
    try {
      ...
      await supabase.from("users").update({
        xp_atual: nextXp.xp_atual,
        nivel: nextXp.nivel,
        xp_proximo_nivel: nextXp.xp_proximo_nivel,
        vida_atual: nextVida,
      ...
      if (nextXp.conquistas.length) {
        await supabase.from("conquistas").upsert(
          nextXp.conquistas.map(c => ({ user_id: uid, tipo: c.tipo, titulo: c.titulo, descricao: c.descricao })),
      ...
      for (let h = 1; h <= fullHours; h++) {
        if (!hourlyStates.some(x => x.hour === h)) hourlyStates.push({ hour: h, mood, at: new Date(new Date(active.startedAt).getTime() + h * 3600000).toISOString() });
      }

      const reward: Reward = { xp, ouro, vida, atributo: "autodominio", atributoDelta, horas: fullHours, niveis: levels };
  ...
  const hoursDisplay = `${pad(Math.floor(elapsedSeconds / 3600))}:${pad(Math.floor((elapsedSeconds % 3600) / 60))}:${pad(elapsedSeconds % 60)}`;
  ...
  return <div className="rpg-panel overflow-hidden border-primary/30 shadow-[0_0_28px_hsl(var(--primary)/.06)]">
    <button type="button" onClick={() => setExpanded(v => !v)} className="w-full p-4 text-left hover:bg-primary/5 transition">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1"><p className="text-[10px] uppercase tracking-[.3em] text-primary">JEJUM</p><h3 className="font-display text-base tracking-widest">{active ? tituloPorHoras(elapsedHours) : "Jejum Atual"}</h3><p className="text-[11px] text-muted-foreground">{active ? `${durationText(elapsedMinutes)} · Em andamento` : sessions.length ? `Último: ${durationText(sessions[0].minutes)}` : "Nenhum jejum registrado"}</p></div>
        <div className="text-right shrink-0"><p className="font-display text-lg text-primary">{active ? durationText(elapsedMinutes) : "—"}</p>{expanded ? <ChevronUp className="w-4 h-4 ml-auto text-muted-foreground" /> : <ChevronDown className="w-4 h-4 ml-auto text-muted-foreground" />}</div>
      </div>
    ...
    <AnimatePresence initial={false}>{expanded && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-border"><div className="p-4 space-y-4">
      {!active ? <>
        ...
        <div className="rounded-md border border-primary/20 bg-primary/5 p-3"><p className="font-display text-sm tracking-widest">RECOMPENSA POR HORA</p><p className="text-[10px] text-muted-foreground mt-1">Cada hora completa sorteia um nível de 1 a 3. Nível 3 é mais raro e rende mais. Menos de 1 hora não gera XP, ouro, vida ou atributo.</p></div>
