...
export default function JejumCard() {
  ...
  useEffect(() => {
    if (!active || elapsedHours < 1) return; const states = [...active.hourlyStates];
    for (let h = 1; h <= elapsedHours; h++) if (!states.some(x => x.hour === h)) states.push({ hour: h, mood, at: new Date(new Date(active.startedAt).getTime() + h * 3600000).toISOString() });
    if (states.length !== active.hourlyStates.length) { const next = { ...active, hourlyStates: states }; setActive(next); if (uid) localStorage.setItem(`${STORAGE(uid)}:active`, JSON.stringify(next)); }
  ...
  return <div className="rpg-panel overflow-hidden border-primary/30 shadow-[0_0_28px_hsl(var(--primary)/.06)]">
    <button type="button" onClick={() => setExpanded(v => !v)} className="w-full p-3 text-left hover:bg-primary/5 transition">
      <div className="flex items-center gap-2.5"><div className="w-9 h-9 rounded-md grid place-items-center bg-primary/10 border border-primary/30 text-primary shrink-0"><Clock3 className="w-4 h-4" /></div><div className="min-w-0 flex-1"><p className="text-[9px] uppercase tracking-[.28em] text-primary">JEJUM</p><h3 className="font-display text-sm tracking-widest">{active ? tituloPorHoras(elapsedHours) : "Jejum Atual"}</h3><p className="text-[10px] text-muted-foreground">{active ? `${durationText(elapsedMinutes)} · Em andamento` : sessions.length ? `Último: ${durationText(sessions[0].minutes)}` : "Nenhum jejum registrado"}</p></div><div className="text-right shrink-0"><p className="font-display text-base text-primary">{active ? durationText(elapsedMinutes) : "—"}</p>{expanded ? <ChevronUp className="w-3.5 h-3.5 ml-auto text-muted-foreground" /> : <ChevronDown className="w-3.5 h-3.5 ml-auto text-muted-foreground" />}</div></div>
      {active && <div className="mt-2"><div className="h-1.5 rounded-full bg-secondary overflow-hidden"><motion.div className="h-full bg-primary" animate={{ width: `${progress}%` }} /></div><div className="flex justify-between mt-1 text-[8px] uppercase tracking-widest text-muted-foreground"><span>{durationText(elapsedMinutes)}</span><span>{nextMilestone ? `Próximo Marco: ${nextMilestone}h` : "Marcos Superados"}</span></div></div>}
    </button>
    ...
    <AnimatePresence initial={false}>{expanded && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-border"><div className="p-3 space-y-3">
      {!active ? <>
        ...
        <div><p className="text-[9px] uppercase tracking-[.25em] text-muted-foreground mb-1.5">Como Está Sua Mente?</p><div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5">{MOODS.map(m => { const Icon = m.icon; const selected = mood === m.id; return <button key={m.id} onClick={() => chooseMood(m.id)} className={`rounded-md border p-1.5 text-[9px] flex items-center justify-center gap-1 transition ${selected ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground hover:border-primary/50"}`}><Icon className="w-3.5 h-3.5" />{m.label}</button>; })}</div></div>
        <UrgeSurfingCard embedded />
        <div className="rounded-md border border-primary/20 bg-primary/5 p-2.5 text-center"><p className="font-display text-xs tracking-widest">VONTADE ≠ COMANDO</p><p className="text-[9px] text-muted-foreground mt-1">Seu estado é registrado ao longo das horas. Você ainda escolhe o próximo passo.</p></div>
