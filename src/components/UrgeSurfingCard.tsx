...
import { Droplets, Waves, TimerReset, TrendingDown, Minus, TrendingUp, Brain, RotateCcw, Trash2, GlassWater } from "lucide-react";
...
export default function UrgeSurfingCard({ embedded = false }: Props) {
  ...
  useEffect(() => {
    if (!uid) return; setWaves(loadWaves(uid)); setWater(loadWater(uid)); setWaterActions(loadActions(uid)); setWaterTarget(loadTarget(uid));
    const sync = () => { setFasting(Boolean(localStorage.getItem(`ascensao:jejum:${uid}:active`))); };
    sync(); const timer = window.setInterval(sync, 1000); const midnight = window.setTimeout(() => { resetDailyWater(); }, msUntilTomorrow());
    return () => { window.clearInterval(timer); window.clearTimeout(midnight); };
  }, [uid]);
