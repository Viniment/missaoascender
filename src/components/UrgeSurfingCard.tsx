...
import { Waves, TrendingDown, Minus, TrendingUp, Brain, RotateCcw, Trash2, GlassWater, Eye, Compass, X, Play } from "lucide-react";
...
const TRIGGERS: { id: Trigger; label: string; emoji: string }[] = [
  { id: "vontade_comer", label: "Vontade de Comer", emoji: "🍔" },
  { id: "vontade_beber", label: "Vontade de Beber", emoji: "🥤" },
  { id: "doce", label: "Vontade de Comer Doce", emoji: "🍫" },
  { id: "comida_vista", label: "Comida À Vista", emoji: "👀" },
  { id: "fome", label: "Sensação de Fome", emoji: "🍽️" },
  { id: "sede", label: "Sensação de Sede", emoji: "💧" },
  { id: "tedio", label: "Tédio", emoji: "😶" },
  { id: "ansiedade", label: "Ansiedade", emoji: "😰" },
  { id: "habito", label: "Hábito / Automático", emoji: "🔁" },
  { id: "pensamento", label: "Pensamento Intrusivo", emoji: "🧠" },
...
const TRIGGER_COPY: Record<Trigger, { noun: string; thought: string; question: string; guidance: string }> = {
  vontade_comer: { noun: "vontade de comer", thought: "Estou percebendo uma vontade de comer agora.", question: "O que minha mente está prometendo que comer vai resolver ou proporcionar?", guidance: "Observe a vontade de comer como uma experiência que surgiu. Você pode notar fome, imagens, pensamentos e sensações sem transformá-los automaticamente em ação." },
  vontade_beber: { noun: "vontade de beber", thought: "Estou percebendo uma vontade de beber agora.", question: "Estou percebendo uma necessidade física, um hábito ou uma interpretação sobre o que preciso neste momento?", guidance: "Observe a sensação de sede e os pensamentos associados. Perceber uma sensação não exige uma resposta automática; você pode investigar antes de escolher." },
  doce: { noun: "vontade de comer doce", thought: "Estou percebendo uma vontade de comer doce agora.", question: "Que promessa esta vontade de doce está trazendo para minha mente?", guidance: "Observe o desejo por doce como uma onda de sensações, imagens e pensamentos. Não é necessário discutir com a vontade; apenas perceba como ela muda." },
  comida_vista: { noun: "vontade ao ver comida", thought: "Estou percebendo uma reação ao ver comida.", question: "A comida está diante de mim; que significado minha mente está dando a esse estímulo?", guidance: "Note o estímulo visual e a resposta automática. Ver comida pode ativar uma resposta aprendida sem determinar o que você fará em seguida." },
  fome: { noun: "sensação de fome", thought: "Estou percebendo uma sensação de fome neste momento.", question: "O que estou sentindo no corpo e que interpretação estou acrescentando a essa sensação?", guidance: "Observe as sensações corporais com curiosidade. Diferencie a percepção física da história que sua mente constrói sobre ela." },
  sede: { noun: "sensação de sede", thought: "Estou percebendo uma sensação de sede neste momento.", question: "O que estou percebendo fisicamente e que significado estou dando a isso?", guidance: "Observe a sensação corporal sem precisar transformá-la imediatamente em uma conclusão. Percepção e interpretação são etapas diferentes." },
  tedio: { noun: "vontade provocada pelo tédio", thought: "Estou percebendo uma vontade que apareceu junto com o tédio.", question: "Estou buscando comida ou outra ação para mudar o estado de tédio?", guidance: "Observe o tédio e a vontade de preenchê-lo. Você pode permitir que o desconforto exista por alguns instantes enquanto decide o que realmente quer fazer." },
  ansiedade: { noun: "vontade associada à ansiedade", thought: "Estou percebendo uma vontade que apareceu junto com a ansiedade.", question: "Estou tentando eliminar a ansiedade rapidamente ou escolher uma resposta para ela?", guidance: "Observe a ansiedade como uma experiência que pode variar. Não é necessário resolver a emoção antes de recuperar sua capacidade de escolher." },
  habito: { noun: "vontade automática de um hábito", thought: "Estou percebendo um impulso automático de repetir um hábito.", question: "O que costuma acontecer depois deste impulso e o que eu quero escolher desta vez?", guidance: "Observe a sequência automática: gatilho → vontade → tendência de ação. Criar um espaço entre esses eventos aumenta sua margem de escolha." },
  pensamento: { noun: "pensamento intrusivo", thought: "Estou percebendo um pensamento que apareceu na minha mente.", question: "Estou tratando este pensamento como um fato ou apenas percebendo que ele apareceu?", guidance: "Pensamentos podem surgir automaticamente. Você pode examiná-los sem precisar aceitá-los como fatos ou obedecê-los." },
  outro: { noun: "vontade que apareceu agora", thought: "Estou percebendo uma experiência interna que pede minha atenção.", question: "Que significado estou dando ao que estou percebendo agora?", guidance: "Observe a experiência com curiosidade. Primeiro perceba o evento; depois examine a interpretação; só então escolha uma resposta." },
};
...
export default function UrgeSurfingCard({ embedded = false }: Props) {
  ...
  const [initial, setInitial] = useState(5); const [finalIntensity, setFinalIntensity] = useState(5); const [trigger, setTrigger] = useState<Trigger>("vontade_comer");
  const [startedAt, setStartedAt] = useState<number | null>(null); const [seconds, setSeconds] = useState(0); const [targetSeconds, setTargetSeconds] = useState(300); const [saving, setSaving] = useState(false); const [choice, setChoice] = useState("");
  ...
  const decreased = todayWaves.filter(w => w.result === "diminuiu").length;
  const decreaseRate = todayWaves.length ? Math.round((decreased / todayWaves.length) * 100) : 0;
  ...
  const startWave = () => {
    if (!fasting) return;
    setInitial(5); setFinalIntensity(5); setTrigger("vontade_comer"); setChoice(""); setStep("perceber"); setTargetSeconds(300); setSeconds(0); setStartedAt(null); setOpen(true);
  };
  ...
  const finishWave = async (result: WaveResult) => {
    if (!uid || !startedAt || seconds < targetSeconds || saving) return;
    setSaving(true);
    try {
  ...
  const waveStats = fasting && <div className="mt-2 grid grid-cols-2 gap-1.5 text-center"><div className="rounded-lg border border-primary/15 bg-primary/[.035] py-2"><p className="text-[8px] text-muted-foreground">🌊 ONDAS HOJE</p><p className="font-display text-sm text-primary">{todayWaves.length}</p></div><div className="rounded-lg border border-primary/15 bg-primary/[.035] py-2"><p className="text-[8px] text-muted-foreground">📉 DIMINUÍRAM</p><p className="font-display text-sm text-primary">{decreaseRate}%</p></div></div>;
  ...
  const modal = <AnimatePresence>{open && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[95] bg-background/90 backdrop-blur-md flex items-center justify-center p-4">
    <motion.div initial={{ y: 24, scale: .97 }} animate={{ y: 0, scale: 1 }} className="relative w-full max-w-lg max-h-[92vh] overflow-y-auto rounded-2xl border border-primary/30 bg-card/95 shadow-[0_0_60px_hsl(var(--primary)/.12)]">
      ...
      <div className="p-5 sm:p-6 space-y-5">
        <div className="text-center"><motion.div animate={{ y: [0, -4, 0] }} transition={{ duration: 2, repeat: Infinity }} className="mx-auto grid place-items-center w-14 h-14 rounded-2xl border border-primary/30 bg-primary/10"><Waves className="w-7 h-7 text-primary" /></motion.div><p className="text-[9px] uppercase tracking-[.32em] text-primary mt-3">🌊 PROTOCOLO DE ONDA</p><h2 className="font-display text-2xl sm:text-3xl tracking-widest mt-1">VONTADE ≠ COMANDO</h2><p className="text-xs text-muted-foreground mt-2">Observe a experiência. Crie distância. Escolha a resposta.</p></div>
        ...
        {step === "perceber" && <motion.div initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
          <div className="rounded-xl border border-primary/20 bg-primary/[.04] p-4 text-center"><p className="text-[9px] uppercase tracking-[.25em] text-primary">👁️ PERCEBA</p><p className="font-display text-lg mt-2">Você percebe uma {copy.noun}.</p><p className="text-xs text-muted-foreground mt-2">{copy.thought}</p></div>
        ...
        {step === "distanciar" && <motion.div initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
          <div className="rounded-xl border border-primary/20 bg-primary/[.04] p-4"><div className="flex items-center gap-2 text-primary"><Brain className="w-4 h-4" /><span className="text-[9px] uppercase tracking-[.25em]">🧠 DISTANCIE</span></div><p className="font-display text-lg mt-3">Você Percebe a {copy.noun.charAt(0).toUpperCase() + copy.noun.slice(1)}.</p><p className="font-display text-xl text-primary mt-1">Você Não Precisa Obedecer a Ela.</p><p className="text-xs text-muted-foreground mt-3">{copy.guidance}</p></div>
          <div className="rounded-xl border border-white/5 bg-white/[.02] p-4"><p className="text-[9px] uppercase tracking-[.25em] text-muted-foreground">🔬 PAUSA METACOGNITIVA</p><p className="text-xs mt-2">“Uma experiência interna pode ser percebida sem ser imediatamente transformada em comportamento.”</p></div>
          <button onClick={() => setStep("investigar")} className="w-full rounded-xl bg-primary py-3 text-xs font-bold tracking-wider text-primary-foreground">CRIEI DISTÂNCIA → CONTINUAR</button>
        </motion.div>}

        {step === "investigar" && <motion.div initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
          <div className="rounded-xl border border-primary/20 bg-primary/[.04] p-4"><div className="flex items-center gap-2 text-primary"><Compass className="w-4 h-4" /><span className="text-[9px] uppercase tracking-[.25em]">🔎 INVESTIGUE</span></div><p className="font-display text-lg mt-3">{copy.question}</p><p className="text-xs text-muted-foreground mt-2">Não procure a resposta “certa”. Observe qual interpretação apareceu primeiro.</p></div>
          <div className="grid grid-cols-1 gap-2">{["É uma necessidade urgente.", "É um pensamento ou impulso que apareceu.", "É uma sensação que posso observar.", "Não tenho certeza ainda."].map(text => <button key={text} onClick={() => setChoice(text)} className={`rounded-xl border p-3 text-left text-xs transition ${choice === text ? "border-primary bg-primary/10 text-primary" : "border-border hover:border-primary/30"}`}>{text}</button>)}</div>
          <button disabled={!choice} onClick={() => setStep("observar")} className="w-full rounded-xl bg-primary py-3 text-xs font-bold tracking-wider text-primary-foreground disabled:opacity-40">OBSERVEI → IR PARA A ONDA</button>
        </motion.div>}

        {step === "observar" && <motion.div initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} className="space-y-5">
          <div className="text-center"><p className="text-[9px] uppercase tracking-[.25em] text-primary">🌊 OBSERVE A ONDA</p><p className="font-display text-5xl text-primary tracking-widest mt-2">{formatTime(seconds)}</p><p className="text-xs text-muted-foreground mt-1">{!startedAt ? `Escolha um período para praticar` : seconds < targetSeconds ? `Observe por mais ${formatTime(targetSeconds - seconds)}` : "Tempo concluído — agora você pode escolher."}</p></div>
          ...
          {!startedAt ? <div className="space-y-3"><p className="text-[10px] uppercase tracking-[.2em] text-muted-foreground text-center">Tempo Mínimo De Prática</p><div className="grid grid-cols-3 gap-2">{[180, 300, 600].map(s => <button key={s} onClick={() => setTargetSeconds(s)} className={`rounded-xl border py-3 text-xs font-semibold transition ${targetSeconds === s ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground hover:border-primary/40"}`}>{s / 60} min</button>)}</div><button onClick={startTimer} className="w-full rounded-xl bg-primary py-3 text-xs font-bold tracking-wider text-primary-foreground flex items-center justify-center gap-2"><Play className="w-4 h-4" /> INICIAR {targetSeconds / 60} MIN</button></div> : <>
            <div className="rounded-xl border border-primary/15 bg-primary/[.035] p-3 space-y-2"><p className="text-[9px] uppercase tracking-[.22em] text-primary">🧭 GUIA DURANTE A PRÁTICA</p><p className="text-[10px] text-muted-foreground">1. Note onde a vontade aparece no corpo.</p><p className="text-[10px] text-muted-foreground">2. Observe pensamentos e imagens sem discutir com eles.</p><p className="text-[10px] text-muted-foreground">3. Perceba se a intensidade muda por conta própria.</p><p className="text-[10px] text-muted-foreground">4. Quando a mente pedir uma ação, apenas reconheça: “isso é uma vontade, não uma ordem”.</p><p className="text-[10px] text-muted-foreground">5. Volte a atenção para a respiração e para as sensações presentes.</p></div>
            ...
            {seconds >= targetSeconds ? <button onClick={() => setStep("escolher")} className="w-full rounded-xl bg-primary py-3 text-xs font-bold tracking-wider text-primary-foreground">🌊 PRÁTICA CONCLUÍDA → ESCOLHER</button> : <div className="text-center text-[9px] text-muted-foreground">Respire naturalmente. Observe sem lutar e sem obedecer automaticamente.</div>}
          </>}
        </motion.div>}

        {step === "escolher" && <motion.div initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
          <div className="rounded-xl border border-primary/20 bg-primary/[.04] p-4 text-center"><p className="text-[9px] uppercase tracking-[.25em] text-primary">🧭 ESCOLHA</p><p className="font-display text-xl mt-2">O que mudou?</p><p className="text-xs text-muted-foreground mt-1">A meta não era eliminar a vontade. Era criar espaço para escolher.</p></div>
        ...
        <div className="rounded-xl border border-white/5 bg-white/[.02] p-3 flex items-center gap-3"><Eye className="w-4 h-4 text-primary shrink-0" /><p className="text-[9px] text-muted-foreground">A prática treina distanciamento: notar uma experiência interna, observar sua interpretação e criar um intervalo antes de agir.</p></div>
      </div>
    </motion.div>
