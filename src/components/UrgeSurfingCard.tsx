...
import { Droplets, Waves, TimerReset, TrendingDown, Minus, TrendingUp, Brain } from "lucide-react";
...
export default function UrgeSurfingCard({ embedded = false }: Props) {
  ...
  const [waves, setWaves] = useState<Wave[]>([]); const [water, setWater] = useState(0); const [waterTarget, setWaterTarget] = useState(DEFAULT_WATER_TARGET);
  ...
  useEffect(() => {
    if (!uid) return; setWaves(loadWaves(uid)); setWater(loadWater(uid)); setWaterTarget(loadTarget(uid));
    const sync = () => setFasting(Boolean(localStorage.getItem(`ascensao:jejum:${uid}:active`))); sync();
  ...
  const addWater = (ml: number) => { if (!uid) return; const next = water + ml; setWater(next); saveWater(uid, next); };
  ...
  const finishWave = (result: WaveResult) => { if (!uid || !startedAt) return; setSaving(true); const wave: Wave = { id: crypto.randomUUID(), date: new Date().toISOString(), initial, final: finalIntensity, durationSeconds: seconds, result, trigger, waterMl: water }; const next = [wave, ...waves]; saveWaves(uid, next); setWaves(next); setStartedAt(null); setOpen(false); setSaving(false); };
  ...
  return <div className="rpg-panel border-primary/20 overflow-hidden"><div className="p-2.5">
    <div className="flex items-center gap-2"><Droplets className="w-4 h-4 text-primary" /><div className="min-w-0 flex-1"><p className="text-[8px] uppercase tracking-[.25em] text-primary">HIDRATAÇÃO DE HOJE</p><p className="font-display text-xs">{water} / {waterTarget} ml</p></div><span className="text-[9px] text-muted-foreground">{Math.round(progress)}%</span></div>
    <div className="h-1.5 rounded-full bg-secondary overflow-hidden mt-1.5"><motion.div className="h-full bg-primary" animate={{ width: `${progress}%` }} /></div>
    <div className="flex gap-1 mt-1.5"><button onClick={() => addWater(100)} className="flex-1 border border-border rounded py-1 text-[8px]">+100</button><button onClick={() => addWater(200)} className="flex-1 border border-border rounded py-1 text-[8px]">+200</button><button onClick={() => addWater(400)} className="flex-1 border border-border rounded py-1 text-[8px]">+400 ml</button></div>
