...
import { BookOpen, CalendarDays, Clock3, FileText, Heart, Lightbulb, PenLine, Trash2, X } from "lucide-react";
...
export default function CantinhoDiario({ area, userId, close }: { area: Area; userId: string; close: () => void }) {
  ...
  const load = async () => {
    setLoading(true);
    const r = await supabase.from("cantinho_diarios" as any)
      .select("id,area_id,titulo,conteudo,data,created_at,updated_at")
      .eq("user_id", userId).eq("area_id", area.id)
      .order("data", { ascending: false }).order("created_at", { ascending: false });
    if (r.error) { alert(`Não foi possível carregar o diário: ${r.error.message}`); setLoading(false); return; }
    const list = (r.data || []) as Diario[];
    setEntries(list);
    setLoading(false);
  };
  ...
  const startNew = () => {
    setSelected(null);
    setTitle(`Diário de ${hoje()}`);
    setContent("");
    setTutorial(false);
    window.setTimeout(() => editorRef.current?.focus(), 80);
  };

  const openEntry = (entry: Diario) => {
    setSelected(entry); setTitle(entry.titulo); setContent(entry.conteudo); setTutorial(false);
    window.setTimeout(() => editorRef.current?.focus(), 80);
  };
  ...
  const save = async () => {
    ...
    const r = selected
      ? await supabase.from("cantinho_diarios" as any).update(payload).eq("id", selected.id).eq("user_id", userId).select("id,area_id,titulo,conteudo,data,created_at,updated_at").single()
      : await supabase.from("cantinho_diarios" as any).insert(payload).select("id,area_id,titulo,conteudo,data,created_at,updated_at").single();
    setSaving(false);
    if (r.error) { alert(`Erro ao salvar diário: ${r.error.message}`); return; }
    const saved = r.data as Diario;
    setSelected(saved); setTitle(saved.titulo); setContent(saved.conteudo);
    await load();
  };

  const del = async () => {
    if (!selected || !window.confirm("Excluir esta entrada do diário? Esta ação não pode ser desfeita.")) return;
    const r = await supabase.from("cantinho_diarios" as any).delete().eq("id", selected.id).eq("user_id", userId);
    if (r.error) { alert(`Erro ao excluir: ${r.error.message}`); return; }
    setSelected(null); setTitle(""); setContent(""); await load();
  };
      ...
      <main className="relative mx-auto max-w-6xl px-4 py-6 sm:px-7 sm:py-8">
        ...
        <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
          ...
          <section className="min-w-0">
            {!selected && !title ? <div className="flex min-h-[55vh] flex-col items-center justify-center rounded-[32px] border border-white/10 bg-white/[.025] p-8 text-center"><div className="grid h-16 w-16 place-items-center rounded-3xl border border-fuchsia-300/20 bg-fuchsia-500/10"><PenLine className="h-7 w-7 text-fuchsia-300" /></div><h2 className="mt-6 font-display text-xl tracking-widest">SEU PRIMEIRO DIA</h2><p className="mt-2 max-w-md text-sm leading-6 text-white/45">Abra uma página e conte como foi um dia da vida que você está construindo.</p><button type="button" onClick={startNew} className="mt-6 rounded-2xl bg-gradient-to-r from-fuchsia-600 to-fuchsia-500 px-6 py-3 text-[10px] font-black uppercase shadow-xl shadow-fuchsia-950/30"><PenLine className="mr-1 inline h-4 w-4" /> Escrever meu primeiro dia</button></div> :
              <div className="rounded-[32px] border border-white/10 bg-white/[.025] p-5 shadow-2xl shadow-fuchsia-950/10 sm:p-7">
                ...
                <div className="rounded-3xl border border-fuchsia-300/10 bg-black/30 p-1 shadow-inner">
                  <textarea ref={editorRef} value={content} onChange={e => setContent(e.target.value)} onClick={() => editorRef.current?.focus()} onFocus={() => setTutorial(false)} placeholder={'Hoje foi um dia...\n\nComece contando como sua manhã começou. Onde você acordou? Quem estava com você? O que aconteceu ao longo do dia?\n\nEscreva como se esta realidade já fosse sua.'} className="pointer-events-auto block min-h-[52vh] w-full cursor-text resize-none rounded-[22px] border-0 bg-transparent px-4 py-4 text-[15px] leading-8 text-white/90 outline-none focus:outline-none focus:ring-2 focus:ring-fuchsia-400/20 placeholder:text-white/20 sm:min-h-[58vh] sm:px-6 sm:py-5" />
                  <div className="flex items-center justify-between border-t border-white/5 px-4 py-3"><span className="text-[9px] text-white/25">{content.trim() ? content.trim().split(/\s+/).length : 0} palavras</span><button type="button" onClick={save} disabled={saving || !content.trim()} className="rounded-xl bg-fuchsia-500 px-5 py-2.5 text-[9px] font-black uppercase text-white shadow-lg shadow-fuchsia-950/30 disabled:opacity-40">{saving ? "Salvando..." : "Guardar este dia"}</button></div>
                </div>
