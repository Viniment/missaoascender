...
export default function RichTextEditor({ value, onChange, placeholder = "Escreva suas anotações...", minHeight = "280px" }: Props) {
  ...
  const syncActive = () => {
    ...
    const matchedFont = fonts.find(font => rawFont.toLowerCase() === font.toLowerCase());
    setCurrentFont(matchedFont || "Arial");
  ...
  const restoreSelection = () => {
    if (!savedSelection.current || !ref.current) return;
    const sel = window.getSelection(); sel?.removeAllRanges(); sel?.addRange(savedSelection.current); syncActive();
  };

  const command = (name: string, arg?: string) => {
    restoreSelection(); exec(name, arg); ref.current?.focus(); emit(); syncActive(); window.setTimeout(rememberSelection, 0);
  };

  const link = () => {
    restoreSelection(); const url = window.prompt("URL do link:", "https://");
    if (url) exec("createLink", url); ref.current?.focus(); emit(); syncActive();
  };

  const insertEmoji = (emoji: string) => {
    restoreSelection(); exec("insertText", emoji); ref.current?.focus(); emit();
    setShowTopEmojis(false); setShowInlineEmojis(false); window.setTimeout(rememberSelection, 0);
  };

  const block = (type: string) => command("formatBlock", type);

  const toggleEmoji = (inline: boolean, el: HTMLElement) => {
    const rect = el.getBoundingClientRect();
    setEmojiPosition({ left: inline ? rect.left + rect.width / 2 : Math.min(rect.left, window.innerWidth - 370), top: rect.bottom + 8, centered: inline });
    if (inline) {
      setShowInlineEmojis(v => !v); setShowTopEmojis(false);
    } else {
      setShowTopEmojis(v => !v); setShowInlineEmojis(false);
    }
    setShowColors(false); setShowHighlights(false);
  };

  const emojiPanel = (inline = false) => <div style={{ left: emojiPosition.left, top: emojiPosition.top, transform: emojiPosition.centered ? "translateX(-50%)" : undefined }} className="fixed z-[2147483647] w-[min(380px,calc(100vw-20px))] overflow-hidden rounded-2xl border border-white/10 bg-[#08080d]/[.98] shadow-[0_20px_70px_rgba(0,0,0,.9)] backdrop-blur-xl">
    <div className="flex items-center justify-between border-b border-white/10 bg-white/[.035] px-3 py-2.5">
      <div><div className="text-[10px] font-black uppercase tracking-[.18em] text-white/70">Emojis</div><div className="mt-0.5 text-[8px] text-white/25">Insira no ponto atual do texto</div></div>
      <span className="rounded-md border border-white/10 bg-black/30 px-1.5 py-1 text-[7px] font-bold uppercase tracking-wider text-white/25">{EMOJI_GROUPS.reduce((n, g) => n + g.emojis.length, 0)}</span>
    </div>
    <div className="emoji-picker-scroll max-h-[70vh] space-y-3 overflow-y-auto p-3">
      {EMOJI_GROUPS.map(group => <div key={group.label}><div className="px-1 pb-1.5 text-[7px] font-black uppercase tracking-[.16em] text-fuchsia-200/45">{group.label}</div><div className="grid grid-cols-8 gap-1">{group.emojis.map((emoji, i) => <button key={`${group.label}-${emoji}-${i}`} type="button" onMouseDown={e => e.preventDefault()} onClick={() => insertEmoji(emoji)} className="grid h-9 w-full place-items-center rounded-lg border border-transparent text-lg transition hover:border-fuchsia-300/15 hover:bg-fuchsia-500/10 hover:scale-105 active:scale-95">{emoji}</button>)}</div></div>)}
    </div>
  </div>;
  ...
  return <div className="overflow-visible rounded-2xl border border-white/10 bg-[#090910] shadow-inner shadow-black/30">
    ...
    <div className="relative">
      {inlineToolbar.visible && <div style={{ left: inlineToolbar.x, top: inlineToolbar.y }} className="absolute z-40 flex items-center gap-1 rounded-xl border border-white/10 bg-[#050507]/[.98] p-1.5 shadow-[0_16px_50px_rgba(0,0,0,.8)] backdrop-blur-xl" onMouseDown={e => e.preventDefault()}>
        ...
        <div className="relative"><ToolButton title="Emoji" onClick={e => toggleEmoji(true, e.currentTarget)}>😀</ToolButton>{showInlineEmojis && emojiPanel(true)}</div>
      </div>}
