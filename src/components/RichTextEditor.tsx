...
export default function RichTextEditor({ value, onChange, placeholder = "Escreva suas anotações...", minHeight = "280px" }: Props) {
  ...
  useEffect(() => {
    const rememberCaret = () => {
      ...
      if (sel?.rangeCount && ref.current?.contains(sel.anchorNode)) savedSelection.current = sel.getRangeAt(0).cloneRange();
    };
  ...
  const rememberSelection = () => {
    ...
    const rect = sel.getRangeAt(0).getBoundingClientRect(); const er = ref.current.getBoundingClientRect();
  ...
  // Block formatting is structural. Choosing a heading replaces previous font-size styling.
  const applyBlock = (type: string) => { restoreSelection(); exec("formatBlock", `<${type}>`); if (type.startsWith("h")) exec("bold"); ref.current?.focus(); emit(); syncActive(); window.setTimeout(rememberSelection,0); };
  // Manual font sizing has priority over a heading: it converts the current block to normal text first.
  ...
  return <div className="overflow-visible rounded-2xl border border-white/10 bg-[#090910] shadow-inner shadow-black/30">
    ...
    <div className="flex flex-wrap items-center gap-1 border-b border-white/10 bg-white/[.025] p-2">
      ...
      <select value={currentBlock} onChange={e=>applyBlock(e.target.value)} className="h-8 rounded-lg border border-white/10 bg-black px-2 text-[9px] font-bold text-white outline-none hover:border-fuchsia-400/30"><option value="p">Texto</option>{blockOptions.slice(1).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select>
