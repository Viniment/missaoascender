...
import { AlignCenter, AlignLeft, AlignRight, Bold, CheckSquare, Code2, Highlighter, Italic, Link2, List, ListOrdered, Minus, Quote, Redo2, Strikethrough, Underline, Undo2 } from "lucide-react";
...
export default function RichTextEditor({ value, onChange, placeholder = "Escreva suas anotações...", minHeight = "280px" }: Props) {
  ...
  const syncActive = () => {
    ...
    setCurrentBlock(validBlocks.includes(block) ? block : "p");
    const rawFont = String(document.queryCommandValue("fontName") || "Arial").replace(/[\"']/g, "").trim();
    const fonts = ["Arial", "Georgia", "Verdana", "Tahoma", "Courier New", "Trebuchet MS"];
    setCurrentFont(fonts.find(font => rawFont.toLowerCase() === font.toLowerCase()) || "Arial");
    const rawSize = String(document.queryCommandValue("fontSize") || "3");
    setCurrentSize(["2", "3", "4", "5", "6"].includes(rawSize) ? rawSize : "3");
  ...
  const rememberSelection = () => {
    ...
    const y = Math.max(8, rect.bottom - editorRect.top + 8); setInlineToolbar({ visible: true, x, y });
  };
  ...
  // formatBlock expects an HTML tag value in browsers such as Chromium.
  ...
  const toggleEmoji = (inline: boolean, el: HTMLElement) => { const rect = el.getBoundingClientRect(); setEmojiPosition({ left: inline ? rect.left + rect.width / 2 : Math.min(rect.left, window.innerWidth - 370), top: rect.bottom + 8, centered: inline }); if (inline) { setShowInlineEmojis(v => !v); setShowTopEmojis(false); } else { setShowTopEmojis(v => !v); setShowInlineEmojis(false); } };
  const emojiPanel = () => <div style={{ left: emojiPosition.left, top: emojiPosition.top, transform: emojiPosition.centered ? "translateX(-50%)" : undefined }} className="fixed z-[2147483647] w-[min(380px,calc(100vw-20px))] overflow-hidden rounded-2xl border border-white/10 bg-[#08080d]/[.98] shadow-[0_20px_70px_rgba(0,0,0,.9)] backdrop-blur-xl"><div className="flex items-center justify-between border-b border-white/10 bg-white/[.035] px-3 py-2.5"><div><div className="text-[10px] font-black uppercase tracking-[.18em] text-white/70">Emojis</div><div className="mt-0.5 text-[8px] text-white/25">Insira no ponto atual do texto</div></div><span className="rounded-md border border-white/10 bg-black/30 px-1.5 py-1 text-[7px] font-bold uppercase tracking-wider text-white/25">{EMOJI_GROUPS.reduce((n, g) => n + g.emojis.length, 0)}</span></div><div className="emoji-picker-scroll max-h-[70vh] space-y-3 overflow-y-auto p-3">{EMOJI_GROUPS.map(group => <div key={group.label}><div className="px-1 pb-1.5 text-[7px] font-black uppercase tracking-[.16em] text-fuchsia-200/45">{group.label}</div><div className="grid grid-cols-8 gap-1">{group.emojis.map((emoji, i) => <button key={`${group.label}-${emoji}-${i}`} type="button" onMouseDown={e => e.preventDefault()} onClick={() => insertEmoji(emoji)} className="grid h-9 w-full place-items-center rounded-lg border border-transparent text-lg transition hover:border-fuchsia-300/15 hover:bg-fuchsia-500/10 hover:scale-105 active:scale-95">{emoji}</button>)}</div></div>)}</div></div>;

  return <div className="overflow-visible rounded-2xl border border-white/10 bg-[#090910] shadow-inner shadow-black/30">
    <style>{`.emoji-picker-scroll::-webkit-scrollbar{width:7px}.emoji-picker-scroll::-webkit-scrollbar-track{background:rgba(255,255,255,.035);border-radius:999px}.emoji-picker-scroll::-webkit-scrollbar-thumb{background:rgba(156,163,175,.55);border-radius:999px}.emoji-picker-scroll::-webkit-scrollbar-thumb:hover{background:rgba(209,213,219,.75)}.emoji-picker-scroll{scrollbar-color:rgba(156,163,175,.6) rgba(255,255,255,.035);scrollbar-width:thin}.rich-notes-editor ul{list-style-type:disc!important;padding-left:1.5rem!important}.rich-notes-editor ol{list-style-type:decimal!important;padding-left:1.5rem!important}.rich-notes-editor li{padding-left:.25rem!important;display:list-item!important}.rich-notes-editor ul ul{list-style-type:circle!important}.rich-notes-editor ul ul ul{list-style-type:square!important}`}</style>
    <div className="flex flex-wrap items-center gap-1 border-b border-white/10 bg-white/[.025] p-2">
      ...
      <EditorQuickColors currentText={currentTextColor} currentBackground={currentBackgroundColor} onText={color => command("foreColor", color)} onBackground={color => command("hiliteColor", color === "transparent" ? "transparent" : color)} />
      ...
      <div className="relative"><ToolButton title="Inserir emoji" onClick={e => toggleEmoji(false, e.currentTarget)}>😀</ToolButton>{showTopEmojis && emojiPanel()}</div>
      <EditorTemplatesMenu value={value} onApply={applyTemplate} />
      <ToolButton title="Linha separadora" onClick={() => command("insertHorizontalRule")}><Minus className="h-3.5 w-3.5" /></ToolButton>
    </div>
    <div className="relative">
      {inlineToolbar.visible && <div style={{ left: inlineToolbar.x, top: inlineToolbar.y }} className="absolute z-40 flex items-center gap-1 rounded-xl border border-white/10 bg-[#050507]/[.98] p-1.5 shadow-[0_16px_50px_rgba(0,0,0,.8)] backdrop-blur-xl" onMouseDown={e => e.preventDefault()}><ToolButton title="Negrito" active={active.bold} onClick={() => command("bold")}><Bold className="h-3.5 w-3.5" /></ToolButton><ToolButton title="Itálico" active={active.italic} onClick={() => command("italic")}><Italic className="h-3.5 w-3.5" /></ToolButton><ToolButton title="Sublinhado" active={active.underline} onClick={() => command("underline")}><Underline className="h-3.5 w-3.5" /></ToolButton><ToolButton title="Tachado" active={active.strikeThrough} onClick={() => command("strikeThrough")}><Strikethrough className="h-3.5 w-3.5" /></ToolButton><EditorQuickColors currentText={currentTextColor} currentBackground={currentBackgroundColor} onText={color => command("foreColor", color)} onBackground={color => command("hiliteColor", color === "transparent" ? "transparent" : color)} /><ToolButton title="Inserir link" onClick={link}><Link2 className="h-3.5 w-3.5" /></ToolButton><div className="relative"><ToolButton title="Emoji" onClick={e => toggleEmoji(true, e.currentTarget)}>😀</ToolButton>{showInlineEmojis && emojiPanel()}</div></div>}
