...
export default function RichTextEditor({ value, onChange, placeholder = "Escreva suas anotações...", minHeight = "280px" }: Props) {
  ...
  const restoreSelection = () => { if (!savedSelection.current) return; const sel = window.getSelection(); sel?.removeAllRanges(); sel?.addRange(savedSelection.current); };
  ...
  return <div className="overflow-hidden rounded-2xl border border-white/10 bg-[#090910] shadow-inner shadow-black/30">
    <div className="flex flex-wrap items-center gap-1 border-b border-white/10 bg-white/[.025] p-2">
      ...
      <ToolButton title="Citação" onClick={() => block("blockquote")}><Quote className="h-3.5 w-3.5" /></ToolButton><ToolButton title="Código" onClick={() => block("pre")}><Code2 className="h-3.5 w-3.5" /></ToolButton><ToolButton title="Inserir link" onClick={link}><Link2 className="h-3.5 w-3.5" /></ToolButton><ToolButton title="Inserir emoji" onClick={emoji}>😀</ToolButton><ToolButton title="Linha separadora" onClick={() => command("insertHorizontalRule")}><Minus className="h-3.5 w-3.5" /></ToolButton>
    </div>
    <div ref={ref} contentEditable suppressContentEditableWarning onInput={emit} onKeyUp={rememberSelection} onMouseUp={rememberSelection} onBlur={() => { emit(); rememberSelection(); }} data-placeholder={placeholder} className="rich-notes-editor w-full overflow-y-auto px-4 py-4 text-[14px] leading-7 text-white/85 outline-none sm:px-5 sm:py-5" style={{ minHeight }} />
