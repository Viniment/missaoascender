...
const GROUPS: { id: Grupo; label: string; icon: any }[] = [
  ...
  { id: "nivel", label: "Nível", icon: Sparkles },
  { id: "consistencia", label: "Consistência", icon: Flame },
  { id: "batalha", label: "Batalha", icon: Swords },
  { id: "riqueza", label: "Riqueza", icon: Coins },
  { id: "pureza", label: "Autodomínio", icon: Shield },
];
...
function loadDiary(uid: string) {
  try {
    return JSON.parse(localStorage.getItem(`ascensao:diario:${uid}`) || "[]") as Array<{ date: string; text: string; type: string }>;
  } catch {
    return [];
  }
}
...
export default function ConquistasPage() {
  ...
  const { data: derrotados } = useQuery({
    ...
    queryFn: async () => {
      const { count } = await supabase.from("inimigo").select("id", { count: "exact", head: true }).eq("user_id", uid!).eq("ativo", false);
      return count ?? 0;
    },
    enabled: !!uid,
  });
  const { data: economia } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("transacoes_ouro").select("valor,origem").eq("user_id", uid!);
      const rows = data ?? [];
      return {
        ouroGanho: rows.filter(r => (r.valor ?? 0) > 0).reduce((s, r) => s + (r.valor ?? 0), 0),
        compras: rows.filter(r => (r.origem ?? "") === "loja").length,
      };
    },
    enabled: !!uid,
  });
  const { data: pureza } = useQuery({
    ...
    queryFn: async () => {
      const { data: logs } = await supabase.from("habito_logs").select("data, completado, habitos!inner(tipo)").eq("user_id", uid!).eq("completado", true);
      ...
      let perfeitos = 0;
      for (const { pos, neg } of byDay.values()) if (pos > 0 && neg === 0) perfeitos++;
      return { perfeitos, semArmadilhas: perfeitos };
    },
    enabled: !!uid,
  });

  const [diary, setDiary] = useState<Array<{ date: string; text: string; type: string }>>([]);
  const [filtro, setFiltro] = useState<Grupo>("todas");
  ...
  const catalog = useMemo<CatalogItem[]>(() => {
    ...
    const ganho = economia?.ouroGanho ?? 0;
    ...
    for (const n of [2, 3, 5, 10, 15, 20, 30, 50, 75, 100]) {
      const u = unlock(`nivel_${n}`, nivel >= n);
      add({ tipo: `nivel_${n}`, titulo: `Nível ${n}`, descricao: `Alcance o nível ${n} e avance mais um capítulo da campanha.`, como: `Continue acumulando XP até o nível ${n}.`, unlocked: u, progresso: u ? undefined : `${nivel}/${n}`, percent: nivel / n * 100, desbloqueada_em: u ? db(`nivel_${n}`) : null, grupo: "nivel", raridade: n >= 50 ? "lendaria" : n >= 20 ? "epica" : n >= 10 ? "rara" : "comum", icon: Sparkles });
    }

    const diaryDates = new Set(diary.map(e => e.date)).size;
    const diaryCount = diary.length;
    const urgeCount = diary.filter(e => e.type === "urge").length;
    ...
    for (const n of [3, 7, 14, 30, 60, 100]) {
      const u = unlock(`streak_${n}`, streak >= n);
      add({ tipo: `streak_${n}`, titulo: `${n} dias de streak`, descricao: `Mantenha ${n} dias seguidos de foco.`, como: `Complete pelo menos um hábito positivo por ${n} dias consecutivos.`, unlocked: u, progresso: u ? undefined : `${streak}/${n}`, percent: streak / n * 100, desbloqueada_em: u ? db(`streak_${n}`) : null, grupo: "consistencia", raridade: n >= 60 ? "lendaria" : n >= 14 ? "epica" : "rara", icon: Flame });
    }
    const bau = !!heroi.ultimo_bau_data || map.has("bau_lendario");
    add({ tipo: "bau_lendario", titulo: "Caçador de recompensas", descricao: "Você descobriu que consistência também pode virar loot.", como: "Abra o baú diário.", unlocked: bau, progresso: bau ? undefined : "0/1", desbloqueada_em: bau ? db("bau_lendario") : null, grupo: "consistencia", raridade: "rara", icon: Star });
    ...
    const batalha = !!inimigo || kills > 0 || map.has("primeira_batalha");
    add({ tipo: "primeira_batalha", titulo: "Primeira batalha", descricao: "Você identificou um inimigo interno e decidiu enfrentá-lo.", como: "Cadastre um inimigo de campanha.", unlocked: batalha, progresso: batalha ? undefined : "0/1", desbloqueada_em: batalha ? db("primeira_batalha") : null, grupo: "batalha", raridade: "comum", icon: Swords });
    for (const n of [1, 3, 5, 10, 25]) {
      const tipo = n === 1 ? "primeiro_inimigo_derrotado" : `inimigos_${n}`;
      const u = unlock(tipo, kills >= n);
      add({ tipo, titulo: n === 1 ? "Primeiro inimigo derrotado" : `${n} inimigos derrotados`, descricao: "Um padrão a menos controlando a campanha.", como: "Zere o HP de inimigos cumprindo seus hábitos positivos.", unlocked: u, progresso: u ? undefined : `${kills}/${n}`, percent: kills / n * 100, desbloqueada_em: u ? db(tipo) : null, grupo: "batalha", raridade: n >= 10 ? "lendaria" : n >= 5 ? "epica" : "rara", icon: Swords });
    }

    for (const n of [100, 500, 1000, 5000, 10000]) {
      const tipo = `ouro_ganho_${n}`;
      const u = ganho >= n;
      add({ tipo, titulo: `${n.toLocaleString("pt-BR")} de ouro ganho`, descricao: `Acumule ${n.toLocaleString("pt-BR")} de ouro ao longo da campanha.`, como: "Cumpra ações que geram ouro.", unlocked: u, progresso: u ? undefined : `${ganho.toLocaleString("pt-BR")}/${n.toLocaleString("pt-BR")}`, percent: ganho / n * 100, desbloqueada_em: u ? db(tipo) : null, grupo: "riqueza", raridade: n >= 5000 ? "lendaria" : n >= 1000 ? "epica" : "rara", icon: Coins });
    }
    for (const n of [500, 2000, 5000]) {
      const tipo = `ouro_cofre_${n}`;
      const u = ouro >= n;
      add({ tipo, titulo: `${n.toLocaleString("pt-BR")} guardados`, descricao: "Você aprendeu a acumular poder sem gastar tudo.", como: "Mantenha esse valor de ouro ao mesmo tempo.", unlocked: u, progresso: u ? undefined : `${ouro}/${n}`, percent: ouro / n * 100, desbloqueada_em: u ? db(tipo) : null, grupo: "riqueza", raridade: n >= 5000 ? "lendaria" : "epica", icon: Coins });
    }
    for (const n of [1, 5, 15, 30]) {
      const tipo = `compras_${n}`;
      const u = compras >= n;
      add({ tipo, titulo: n === 1 ? "Primeira compra" : `${n} compras`, descricao: "Transforme ouro em recursos para sua evolução.", como: "Compre itens na loja.", unlocked: u, progresso: u ? undefined : `${compras}/${n}`, percent: compras / n * 100, desbloqueada_em: u ? db(tipo) : null, grupo: "riqueza", raridade: n >= 15 ? "epica" : "comum", icon: Coins });
    }

    for (const n of [1, 5, 15, 30]) {
      const tipo = `dias_perfeitos_${n}`;
      const u = (pureza?.perfeitos ?? 0) >= n;
      add({ tipo, titulo: n === 1 ? "Primeiro dia limpo" : `${n} dias limpos`, descricao: "Você escolheu construir sem alimentar a armadilha.", como: "Complete um hábito positivo e nenhum negativo no dia.", unlocked: u, progresso: u ? undefined : `${pureza?.perfeitos ?? 0}/${n}`, percent: (pureza?.perfeitos ?? 0) / n * 100, desbloqueada_em: u ? db(tipo) : null, grupo: "pureza", raridade: n >= 15 ? "epica" : "rara", icon: Shield });
    }
    return items;
  ...
  // Prioridade de incentivo: primeiro vêm as conquistas ainda não obtidas,
  // ordenadas pela porcentagem concluída (mais próximas primeiro). Depois entram
  // as já obtidas, mantendo as mais recentes/altas primeiro sem competir com os próximos objetivos.
  const ordenarPorProximidade = (a: CatalogItem, b: CatalogItem) => {
    ...
    if (a.unlocked && b.unlocked) {
      const da = a.desbloqueada_em ? new Date(a.desbloqueada_em).getTime() : 0;
      const db = b.desbloqueada_em ? new Date(b.desbloqueada_em).getTime() : 0;
  ...
  const filtrado = useMemo(() => {
    const base = filtro === "todas" ? catalog : catalog.filter(c => c.grupo === filtro);
    return [...base].sort(ordenarPorProximidade);
  }, [catalog, filtro]);
  const proxima = filtrado.find(c => !c.unlocked) ?? catalog.filter(c => !c.unlocked).sort(ordenarPorProximidade)[0];
  ...
  return <Shell><div className="mx-auto max-w-6xl space-y-5 pb-4">
    <header className="relative overflow-hidden rounded-3xl border border-primary/30 bg-card p-5 shadow-[0_0_40px_hsl(var(--primary)/0.08)] sm:p-7">
      ...
      <div className="relative flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.28em] text-primary"><Trophy className="h-4 w-4" /> Sala de Troféus</div>
          <h1 className="mt-2 font-display text-2xl tracking-[0.14em] text-primary glow-text-purple sm:text-3xl">CONQUISTAS</h1>
          <p className="mt-2 max-w-2xl text-xs leading-relaxed text-muted-foreground">Cada conquista representa uma mudança real: aprender, observar, escolher e agir. A campanha não mede só resultado — mede evolução.</p>
        </div>
        <div className="w-full shrink-0 rounded-2xl border border-primary/30 bg-primary/10 px-5 py-3 text-center sm:w-auto"><div className="font-display text-2xl text-primary">{obtidas}/{total}</div><div className="text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground">desbloqueadas</div></div>
      </div>
    ...
    {proxima && <section className="rpg-panel flex items-center gap-3 border-primary/25 p-4"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-primary/10 text-primary"><Target className="h-5 w-5" /></div><div className="min-w-0 flex-1"><div className="text-[9px] font-black uppercase tracking-[0.22em] text-primary">PRÓXIMA CONQUISTA</div><div className="mt-0.5 truncate font-display text-sm tracking-wider">{proxima.titulo}</div><div className="mt-1 h-1.5 overflow-hidden rounded-full bg-muted"><div className="h-full bg-primary" style={{ width: `${proxima.percent}%` }} /></div></div><span className="shrink-0 text-[10px] font-black text-primary">{Math.round(proxima.percent ?? 0)}%</span></section>}

    <section className="rounded-2xl border border-border/60 bg-card/30 p-2 sm:p-3">
      <div className="mb-2 px-1 text-[9px] font-black uppercase tracking-[0.22em] text-muted-foreground">Categorias da campanha</div>
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7">
        {GROUPS.map(g => {
          const Icon = g.icon;
          const count = g.id === "todas" ? catalog.length : catalog.filter(c => c.grupo === g.id).length;
          const got = g.id === "todas" ? obtidas : catalog.filter(c => c.grupo === g.id && c.unlocked).length;
          const active = filtro === g.id;
          return <button key={g.id} onClick={() => setFiltro(g.id)} className={`min-w-0 rounded-xl border px-2 py-2.5 transition sm:px-3 ${active ? "border-primary bg-primary/15 text-primary shadow-[0_0_14px_hsl(var(--primary)/0.18)]" : "border-border/60 bg-card/40 text-muted-foreground hover:border-primary/40 hover:text-foreground"}`}>
            <span className="flex min-w-0 items-center justify-center gap-1.5 text-[9px] font-black uppercase tracking-wide sm:text-[10px] sm:tracking-wider"><Icon className="h-3.5 w-3.5 shrink-0" /><span className="truncate">{g.label}</span><span className="shrink-0 opacity-60">{got}/{count}</span></span>
          </button>;
        })}
      </div>
    </section>

    <div className="flex items-center justify-between gap-3 px-1"><div><p className="text-[9px] font-black uppercase tracking-[0.22em] text-primary">Objetivos mais próximos</p><p className="text-[11px] text-muted-foreground">As conquistas que estão quase ao seu alcance aparecem primeiro.</p></div><span className="shrink-0 rounded-full border border-primary/20 bg-primary/5 px-2.5 py-1 text-[9px] font-bold text-primary">{filtrado.filter(c => !c.unlocked).length} restantes</span></div>

    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {filtrado.map(c => {
        const open = aberta === c.tipo;
        const R = RARITY[c.raridade];
        const Icon = c.icon;
        return <motion.button key={c.tipo} layout onClick={() => setAberta(open ? null : c.tipo)} className={`group relative overflow-hidden rounded-2xl border bg-card/70 p-4 text-left transition hover:-translate-y-0.5 ${c.unlocked ? `${R.glow} ${R.className}` : "border-border/60 opacity-70 hover:opacity-100"}`}>
          <div className="flex items-start gap-3"><div className={`grid h-12 w-12 shrink-0 place-items-center rounded-xl border ${c.unlocked ? R.className : "border-border text-muted-foreground"} bg-background/40`}>{c.unlocked ? <Icon className="h-6 w-6" /> : <Lock className="h-5 w-5" />}</div><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><span className={`text-[8px] font-black tracking-[0.16em] ${R.className}`}>{R.label}</span>{c.unlocked && <CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-400" />}</div><h3 className={`mt-1 font-display text-sm tracking-wider ${c.unlocked ? "text-foreground" : "text-muted-foreground"}`}>{c.titulo}</h3><p className="mt-1 text-[11px] leading-relaxed text-muted-foreground">{c.descricao}</p></div></div>
