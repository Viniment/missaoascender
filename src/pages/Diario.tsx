...
import { ArrowLeft, ArrowRight, Brain, Check, ChevronRight, Coins, History, Sparkles, Target, Trophy } from "lucide-react";
...
export default function Diario() {
  ...
  return <Shell><div className="mx-auto max-w-6xl space-y-6">
    ...
    <section className="rounded-3xl border border-border bg-card p-5 shadow-lg sm:p-7"><div className="flex items-center gap-3"><History className="h-5 w-5 text-primary" /><h2 className="text-xl font-black">Histórico de campanha</h2></div><div className="mt-4 space-y-3">{entries.length === 0 ? <div className="rounded-2xl border border-dashed border-border p-5 text-center text-sm text-muted-foreground">Seu primeiro registro começa aqui.</div> : entries.slice(0, 6).map((e) => <div key={e.id} className="rounded-2xl border border-border bg-muted/20 p-4"><div className="flex items-center justify-between gap-2"><span className="text-[10px] font-black uppercase tracking-wider text-primary">{formatBRDate(e.date)}</span><span className="text-[10px] font-black">+{e.xp} XP • +{e.gold} 🪙</span></div><p className="mt-2 whitespace-pre-line line-clamp-4 text-sm">{e.text}</p></div>)}</div></section>
  </div></Shell>;
}
