...
export default function Dashboard() {
  ...
  const { data: quantidadeLogs } = useQuery({
    queryKey: ["quantidade-acoes", uid, dataSelecionada],
    queryFn: () => fetchQuantidadeAcoes(uid!, dataSelecionada),
    enabled: !!uid,
  });
  ...
  useEffect(() => {
    if (!heroi) return;
    if (prevNivel.current !== null && heroi.nivel > prevNivel.current) setLevelUp(heroi.nivel);
    prevNivel.current = heroi.nivel;
  }, [heroi?.nivel]);

  useEffect(() => {
    if (!inimigo) return;
    if (prevEnemyHp.current !== null && prevEnemyHp.current > 0 && inimigo.hp_atual <= 0) setVictory(inimigo.nome);
  ...
  useEffect(() => {
    ...
    supabase.functions.invoke("mensagem-reforco", {
      body: { heroi_nome: heroi.nome, inimigo_nome: inimigo.nome, sonho: (ob as any)?.sonho ?? null, streak: heroi.streak_atual, habito_nome: null, hp_atual: inimigo.hp_atual, hp_max: inimigo.hp_max },
  ...
  const onToggle = async (habitoId: string) => {
    ...
    const tipoTarefa = (h as any).tipo_tarefa ?? "unica";
    const meta = Math.max(1, Number((h as any).quantidade_meta ?? 1));
    const porQuantidade = tipoTarefa === "quantidade" && meta > 1;
    const quantidadeAtual = quantidadeLogs?.get(h.id)?.quantidadeAtual ?? 0;
    const marcado = porQuantidade ? quantidadeAtual >= meta : (logs?.has(h.id) ?? false);
    const positivo = h.tipo === "positivo";
    if (porQuantidade && marcado) {
      toast.info(`Meta de ${meta}x já concluída hoje.`);
      return;
    }
    try {
      const resultado = porQuantidade
        ? await executarAcaoBatalha({ heroi, inimigo: inimigo ?? null, habito: h, marcado: false, quantidadeAtual, data: dataSelecionada })
        : await toggleHabito({ heroi, inimigo: inimigo ?? null, habito: h, marcado, data: dataSelecionada });
      await qc.invalidateQueries();
      if (marcado && !porQuantidade) return;

      fireHabitFX(positivo ? "positive" : "negative");
      const vidaDelta = positivo ? 0 : -Math.max(2, Math.round(h.peso_dano_cura / 3));
      setBattle({ positivo, habito: h.nome, xp: h.peso_xp, dano: h.peso_dano_cura, vidaDelta, msg: "", loading: true });
      ...
      if (positivo) {
        ...
        supabase.functions.invoke("mensagem-reforco", {
          body: { heroi_nome: heroi.nome, inimigo_nome: inimigo?.nome, sonho: (ob as any)?.sonho ?? null, streak: heroi.streak_atual, habito_nome: h.nome, hp_atual: (inimigo?.hp_atual ?? 100) - h.peso_dano_cura, hp_max: inimigo?.hp_max ?? 100 },
        ...
        supabase.functions.invoke("mensagem-inimigo", {
          body: { heroi_nome: heroi.nome, inimigo_nome: inimigo?.nome, sonho: (ob as any)?.sonho ?? null, streak: heroi.streak_atual, habito: h.nome, mentiras: inimigo?.mentiras ?? [] },
  ...
  const abrirBau = async () => {
    ...
    setChestGold(null); setChestOpen(true);
  };

  const executarAberturaBau = async () => {
    if (!heroi) return;
    const g = await abrirBauDiario(heroi.id, heroi);
    setChestGold(g); fireReward(`+${g} ouro`, "#facc15"); await qc.invalidateQueries();
    const infinito = typeof window !== "undefined" && localStorage.getItem("dev_bau_infinito") === "1";
    if (infinito) { await supabase.from("users").update({ ultimo_bau_data: null }).eq("id", heroi.id); await qc.invalidateQueries(); }
  };
    ...
    try {
      ...
      try {
        const { data } = await supabase.functions.invoke("sugerir-pesos-habito", {
          body: { habito_nome: novoHabito.nome.trim(), tipo: novoHabito.tipo, inimigo: inimigo ? { nome: inimigo.nome, gatilho: inimigo.gatilho, mentiras: inimigo.mentiras } : null, onboarding: ob ?? null },
        });
      ...
      const { error } = await supabase.from("habitos").insert({
        ...
        tipo_tarefa: novoHabito.tipoTarefa,
        quantidade_meta: novoHabito.tipoTarefa === "quantidade" ? Math.max(1, Math.floor(novoHabito.quantidadeMeta)) : 1,
        peso_dano_cura, peso_xp, peso_ouro,
      });
      if (error) throw error;
      setNovoHabito({ nome: "", tipo: "positivo", tipoTarefa: "unica", quantidadeMeta: 1 });
      setShowForm(false);
      if (novoHabito.tipo === "positivo") { await recalcularHpMaxInimigo(uid); await qc.invalidateQueries({ queryKey: ["inimigo", uid] }); }
      await qc.invalidateQueries({ queryKey: ["habitos", uid] });
    } catch (e: any) { toast.error(e.message ?? "Erro ao criar ação"); }
    finally { setCreating(false); }
  };

  const excluirHabito = async (id: string) => {
    await supabase.from("habitos").update({ ativo: false }).eq("id", id);
    if (uid) { await recalcularHpMaxInimigo(uid); await qc.invalidateQueries({ queryKey: ["inimigo", uid] }); }
    await qc.invalidateQueries({ queryKey: ["habitos", uid] });
  };
      ...
      <div className="space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: "easeOut" }} className="player-card scanlines p-3.5 sm:p-5">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            <Link to="/personalizar" className="shrink-0 hover:scale-105 transition-transform relative"><div className="avatar-ring"><div className="avatar-inner"><Avatar equipado={heroi.avatar_equipado} size="md" /></div></div><div className="level-badge level-badge-sm absolute -bottom-2 -right-2 shadow-lg"><div className="flex flex-col items-center justify-center px-1"><small>LVL</small><span className="text-sm sm:text-base leading-none">{heroi.nivel}</span></div></div></Link>
            <div className="flex-1 min-w-0 space-y-1"><p className="text-[9px] sm:text-[10px] text-primary/80 uppercase tracking-[0.3em] flex items-center gap-1.5"><Shield className="w-3 h-3" /> HERÓI</p><h2 className="font-display text-lg sm:text-2xl tracking-widest text-foreground glow-text-purple truncate leading-tight">{heroi.nome}</h2>{heroi.titulo && <div className="stat-chip title"><Sparkles className="w-3 h-3" /> {heroi.titulo}</div>}<div className="flex flex-wrap items-center gap-1.5 sm:gap-2 pt-1"><span className="stat-chip gold"><Coins className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> <AnimatedCounter value={heroi.ouro} /></span><span className="stat-chip streak"><Flame className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> {heroi.streak_atual}d</span><Link to="/conquistas" className="stat-chip trophy hover:brightness-125 transition"><Trophy className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> {conquistas?.length ?? 0}</Link></div></div>
          </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between"><h3 className="font-display tracking-[0.3em] text-sm text-primary glow-text-purple">⚔ AÇÕES DE BATALHA</h3><button onClick={() => setShowForm(v => !v)} className="text-[11px] flex items-center gap-1 btn-pixel px-3 py-1.5 rounded-md"><Plus className="w-3 h-3" /> Novo</button></div>
          <div className="rpg-panel p-2 flex items-center gap-2"><button onClick={() => setDataSelecionada(shiftISO(dataSelecionada, -1))} className="p-1.5 rounded-md border border-border hover:border-primary/60 hover:text-primary" title="Dia anterior"><ChevronLeft className="w-4 h-4" /></button><div className="flex-1 text-center"><p className="text-[9px] uppercase tracking-[0.3em] text-muted-foreground flex items-center justify-center gap-1"><Calendar className="w-3 h-3" /> {isHoje ? "Hoje" : "Registro retroativo"}</p><p className="font-display text-sm tracking-widest capitalize">{formatBRDate(dataSelecionada)}</p></div><button onClick={() => setDataSelecionada(shiftISO(dataSelecionada, 1))} disabled={isHoje} className="p-1.5 rounded-md border border-border hover:border-primary/60 hover:text-primary disabled:opacity-30 disabled:cursor-not-allowed" title="Próximo dia"><ChevronRight className="w-4 h-4" /></button>{!isHoje && <button onClick={() => setDataSelecionada(todayISO())} className="text-[10px] px-2 py-1 rounded-md border border-primary/40 text-primary hover:bg-primary/10 uppercase tracking-widest">Hoje</button>}</div>
          ...
          {showForm && <div className="rpg-panel p-4 space-y-3">
            <input className="w-full bg-secondary border border-border rounded-md px-3 py-2 text-sm" placeholder="Ex: Escovar os dentes, Arrumar o quarto..." value={novoHabito.nome} onChange={e => setNovoHabito({ ...novoHabito, nome: e.target.value })} autoFocus />
            <select className="w-full bg-secondary border border-border rounded-md px-3 py-2 text-sm" value={novoHabito.tipo} onChange={e => setNovoHabito({ ...novoHabito, tipo: e.target.value as any })}><option value="positivo">⚔ Positivo — fere o inimigo</option><option value="negativo">💀 Negativo — te fere</option></select>
            <div>
              <label className="text-[10px] uppercase tracking-widest text-muted-foreground">Tipo da tarefa</label>
              <select className="mt-1 w-full bg-secondary border border-border rounded-md px-3 py-2 text-sm" value={novoHabito.tipoTarefa} onChange={e => setNovoHabito({ ...novoHabito, tipoTarefa: e.target.value as any })}>
                <option value="unica">⚔ Única — concluir uma vez</option>
                <option value="quantidade">🔢 Quantidade — repetir várias vezes</option>
              </select>
            </div>
            {novoHabito.tipoTarefa === "quantidade" && <div className="rounded-md border border-primary/30 bg-primary/5 p-3"><label className="text-[10px] uppercase tracking-widest text-primary">Meta diária</label><div className="flex items-center gap-3 mt-1"><input type="number" min={1} max={99} className="w-24 bg-secondary border border-border rounded-md px-3 py-2 text-sm text-center" value={novoHabito.quantidadeMeta} onChange={e => setNovoHabito({ ...novoHabito, quantidadeMeta: Math.max(1, Number(e.target.value) || 1) })} /><span className="text-xs text-muted-foreground">vezes por dia</span></div><p className="text-[10px] text-muted-foreground mt-2">Ex.: 3 → <b>0 / 3</b> → <b>1 / 3</b> → <b>2 / 3</b> → <b>3 / 3</b></p></div>}
            <p className="text-[10px] text-muted-foreground flex items-center gap-1"><Sparkles className="w-3 h-3 text-primary" /> A IA calcula o peso baseado no seu inimigo e no seu sonho.</p>
            <button onClick={criarHabito} disabled={creating || (novoHabito.tipoTarefa === "quantidade" && novoHabito.quantidadeMeta < 1)} className="w-full btn-pixel py-2 rounded-md text-sm disabled:opacity-50 flex items-center justify-center gap-2">{creating ? <><Loader2 className="w-4 h-4 animate-spin" /> Forjando...</> : "Criar ação"}</button>
          </div>}
...
function Bar({ label, pct, value, fillClass, icon }: { label: string; pct: number; value: string; fillClass: string; icon: React.ReactNode }) {
  return <div><div className="flex items-center justify-between mb-1.5"><span className="bar-label">{icon}{label}</span><span className="bar-value">{value}</span></div><div className="bar-track"><div className="bar-fill-wrap"><motion.div className={`relative h-full ${fillClass}`} initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.9, ease: [0.16, 1, 0.3, 1] }}><div className="bar-shine" /><div className="bar-sheen" /></motion.div></div></div></div>;
}
...
function HabitoRow({ h, marcado, quantidadeAtual = 0, quantidadeMeta = 1, onToggle, onDelete, onEdit, negativo }: any) {
  ...
  return <div className={`rpg-panel p-3 flex items-center gap-3 transition-all ${concluida ? "opacity-60" : "hover:border-primary/50"}`}>
    <button onClick={onToggle} disabled={porQuantidade && concluida} className={`w-9 h-9 rounded-md border-2 flex items-center justify-center transition-all shrink-0 ${concluida ? negativo ? "bg-destructive border-destructive shadow-[0_0_10px_hsl(0_90%_55%/0.7)]" : "bg-primary border-primary shadow-[0_0_10px_hsl(var(--primary)/0.7)]" : "border-border hover:border-primary"}`}>
      {porQuantidade ? <span className="text-[11px] font-bold">{concluida ? "✓" : "+1"}</span> : concluida && <span className="text-sm text-primary-foreground font-bold">✓</span>}
    </button>
    <div className="flex-1 min-w-0"><div className="flex items-center gap-2"><p className={`text-sm truncate ${concluida ? "line-through" : ""}`}>{h.nome}</p>{porQuantidade && <span className="shrink-0 text-[10px] font-display tracking-widest text-primary border border-primary/30 rounded px-1.5 py-0.5">{quantidadeAtual} / {quantidadeMeta}</span>}</div><p className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground font-display">{negativo ? <>−{h.peso_xp} XP · +{h.peso_dano_cura} HP inim.</> : <>+{h.peso_xp} XP · −{h.peso_dano_cura} HP · +{h.peso_ouro ?? 0}<span className="text-gold">🪙</span></>}</p>{porQuantidade && <p className="text-[9px] text-muted-foreground mt-0.5">Cada toque registra 1 execução · reinicia diariamente</p>}</div>
    {onEdit && <button onClick={onEdit} className="text-muted-foreground hover:text-primary p-1" title="Editar"><Pencil className="w-3.5 h-3.5" /></button>}
    <button onClick={onDelete} className="text-muted-foreground hover:text-destructive p-1"><Trash2 className="w-3.5 h-3.5" /></button>
  </div>;
}

function BattleOverlay({ battle, onClose, inimigo }: { battle: Battle | null; onClose: () => void; inimigo: any }) {
  return <AnimatePresence>{battle && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] flex items-center justify-center bg-background/80 backdrop-blur-sm p-4"><motion.div initial={{ scale: 0.85, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0 }} transition={{ type: "spring", stiffness: 260, damping: 22 }} className={`relative w-full max-w-sm rpg-panel scanlines p-6 space-y-4 ${battle.positivo ? "neon-glow" : "danger-glow border-destructive/50"}`}>
    ...
    <div className="text-center space-y-1"><p className={`text-[10px] uppercase tracking-[0.4em] ${battle.positivo ? "text-primary" : "text-destructive"}`}>{battle.positivo ? "⚔ GOLPE CERTEIRO" : "💀 O INIMIGO AVANÇA"}</p><h3 className="font-display text-lg tracking-widest text-foreground">{battle.habito}</h3></div>
    <div className="flex items-center justify-center gap-6 py-3">{battle.positivo ? <><div className="text-center animate-damage-pop"><p className="text-[9px] uppercase tracking-widest text-muted-foreground">Dano</p><p className="font-display text-3xl text-destructive glow-text-purple">−{battle.dano}</p><p className="text-[10px] text-muted-foreground">HP {inimigo?.nome ?? "inimigo"}</p></div><div className="text-center animate-damage-pop" style={{ animationDelay: "80ms" }}><p className="text-[9px] uppercase tracking-widest text-muted-foreground">XP</p><p className="font-display text-3xl text-primary glow-text-purple">+{battle.xp}</p><p className="text-[10px] text-muted-foreground">Herói</p></div></> : <><div className="text-center animate-damage-pop"><p className="text-[9px] uppercase tracking-widest text-muted-foreground">Vida</p><p className="font-display text-3xl text-destructive">{battle.vidaDelta}</p><p className="text-[10px] text-muted-foreground">Herói</p></div><div className="text-center animate-damage-pop" style={{ animationDelay: "80ms" }}><p className="text-[9px] uppercase tracking-widest text-muted-foreground">XP</p><p className="font-display text-3xl text-destructive">−{battle.xp}</p><p className="text-[10px] text-muted-foreground">Perdido</p></div></>}</div>
    <div className={`rounded-md border p-3 text-sm italic text-center min-h-[56px] flex items-center justify-center ${battle.positivo ? "border-primary/40 bg-primary/5 text-foreground" : "border-destructive/40 bg-destructive/5 text-foreground/90"}`}>{battle.loading ? <Loader2 className="w-4 h-4 animate-spin text-muted-foreground" /> : `"${battle.msg}"`}</div>
    <button onClick={onClose} className="w-full btn-pixel py-2.5 rounded-md text-xs">Tocar para continuar</button>
