...
function MediaCard({m,onEdit,onDelete}:{m:Media;onEdit:()=>void;onDelete:()=>void}) { const embed = youtubeEmbed(m.src); return <article className="rpg-panel overflow-hidden"><div className="relative aspect-video bg-black/30">{m.tipo === "foto" && <img src={m.src} alt={m.titulo} className="h-full w-full object-cover" />}{m.tipo === "video" && (embed ? <iframe width="560" height="315" src={embed} title={m.titulo || "YouTube video player"} frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen className="h-full w-full" /> : <video src={m.src} controls className="h-full w-full object-contain" />)}{m.tipo === "musica" && (embed ? <iframe width="560" height="315" src={embed} title={m.titulo || "YouTube video player"} frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen className="h-full w-full" /> : <div className="flex h-full items-center justify-center"><audio src={m.src} controls className="w-[90%]" /></div>)}</div><div className="p-4"><div className="flex items-start gap-2"><div className="min-w-0 flex-1"><h3 className="font-display text-sm tracking-wider">{m.titulo || "Sem título"}</h3>{m.legenda && <p className="mt-1 text-xs text-foreground/90">{m.legenda}</p>}{m.significado && <p className="mt-2 border-l-2 border-primary/40 pl-2 text-[11px] leading-relaxed text-muted-foreground">{m.significado}</p>}</div><div className="flex shrink-0"><button onClick={onEdit} className="p-1.5 text-muted-foreground hover:text-primary"><Pencil className="h-3.5 w-3.5" /></button><button onClick={onDelete} className="p-1.5 text-muted-foreground hover:text-destructive"><Trash2 className="h-3.5 w-3.5" /></button></div></div></div></article>; }
...
function EnsaioModal({areas,media,onClose}:{areas:Area[];media:Media[];onClose:()=>void}) {
  ...
  const item=all[i];
  const embed=item ? youtubeEmbed(item.src) : null;
  return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-3"><div className="relative w-full max-w-3xl overflow-hidden rounded-2xl border border-primary/30 bg-background shadow-[0_0_60px_hsl(var(--primary)/0.18)]"><button onClick={onClose} className="absolute right-3 top-3 z-10 rounded-full bg-black/60 p-2 text-white"><X className="h-4 w-4"/></button>{item ? <><div className="relative aspect-video w-full bg-black">{embed?<iframe key={item.id} width="560" height="315" src={embed} title={item.titulo||"YouTube video player"} frameBorder="0" className="absolute inset-0 h-full w-full border-0" loading="eager" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen/>:item.tipo==="foto"?<img src={item.src} className="h-full w-full object-contain"/>:item.tipo==="video"?<video key={item.id} src={item.src} controls autoPlay playsInline className="h-full w-full"/>:item.tipo==="musica"?<div className="flex h-full items-center justify-center p-6"><audio key={item.id} src={item.src} controls autoPlay className="w-full"/> </div>:<div className="flex h-full items-center justify-center p-6 text-center text-xs text-muted-foreground">Não foi possível reconhecer este link do YouTube.</div>}</div><div className="p-5"><div className="text-[9px] font-black uppercase tracking-[0.25em] text-primary">{item.area.emoji} {item.area.nome}</div><h2 className="mt-1 font-display text-lg tracking-widest">{item.titulo||"Meu porquê"}</h2>{item.legenda&&<p className="mt-2 text-sm">{item.legenda}</p>}{item.significado&&<p className="mt-3 border-l-2 border-primary/50 pl-3 text-xs leading-relaxed text-muted-foreground">{item.significado}</p>}<div className="mt-5 flex justify-between"><button disabled={i===0} onClick={()=>setI(v=>v-1)} className="rounded-xl border border-border px-4 py-2 text-xs font-bold disabled:opacity-30">Anterior</button><span className="self-center text-[10px] font-bold text-muted-foreground">{i+1} / {all.length}</span><button disabled={i===all.length-1} onClick={()=>setI(v=>v+1)} className="rounded-xl bg-primary px-4 py-2 text-xs font-black text-primary-foreground disabled:opacity-30" >Próximo</button></div></div></>:<div className="p-10 text-center"><p className="text-sm">Adicione conteúdos ao seu Cantinho primeiro.</p></div>}</div></div>;
}
